...
* ``fork``     — ``src.models.face_detect`` reconstructs 5 points from Apple
  Vision face-landmark *contours* (eye centers, nose = last nose-contour point,
  mouth corners = min/max-x of outerLips). See ``face_detect.py``.
...
def main() -> int:
    ...
    # --- run both pipelines, match, embed ---
    drift: list[float] = []
    n_up = n_fork = n_matched = n_up_only = n_fork_only = 0
    n_fork_minscore = 0  # fork faces passing Immich default minScore
    # matched-face embedding tables (for top-1 identity retrieval)
    ...
    for idx, s in enumerate(samples):
        ...
        fks = fork_faces(s.data)
        n_up += len(ups)
        n_fork += len(fks)
        n_fork_minscore += sum(1 for f in fks if f["score"] >= IMMICH_MIN_SCORE)

        pairs = greedy_match([u["bbox"] for u in ups], [f["bbox"] for f in fks], args.iou)
        n_matched += len(pairs)
        ...
        if (idx + 1) % 25 == 0:
            print(f"  [{idx + 1}/{len(samples)}] matched faces so far: {len(drift)}")
    ...
    emit(f"  upstream (SCRFD) faces : {n_up}")
    emit(f"  fork (Vision) faces    : {n_fork}  (>= minScore {IMMICH_MIN_SCORE}: {n_fork_minscore})")
    emit(f"  matched (IoU>={args.iou})    : {n_matched}")
    emit(f"  upstream-only          : {n_up_only}  (SCRFD found, Vision missed)")
    emit(f"  fork-only              : {n_fork_only}  (Vision found, SCRFD missed)")
    if n_up:
        emit(f"  Vision recall vs SCRFD : {n_matched / n_up:.3f}")
    emit()
    ...
    if labelled and up_emb:
        emit(f"  upstream→upstream (index as-is)      : {up_acc:.4f}")
        emit(f"  fork→fork (full re-index by fork)    : {fk_acc:.4f}")
        emit(f"  fork→upstream (PRESERVE test)        : {cross_acc:.4f}")
        if np.isfinite(up_acc) and np.isfinite(cross_acc):
            emit(f"  preserve drop (upstream - cross)     : {up_acc - cross_acc:+.4f}")
    else:
    ...
    top1_ok = True
    if labelled and up_emb and np.isfinite(up_acc) and np.isfinite(cross_acc):
        top1_ok = (up_acc - cross_acc) <= args.max_top1_drop
    if not np.isfinite(d["median"]):
        emit("VERDICT: INCONCLUSIVE — no matched faces. Check IoU / detectors / data.")
        rc = 2
    elif preserve_ok and top1_ok:
        emit(f"VERDICT: PRESERVE existing face index & clusters (median drift {d['median']:.4f} >= {args.min_cos}).")
        ...
        emit(f"VERDICT: RE-RUN face recognition (median drift {d['median']:.4f}; preserve_ok={preserve_ok}, top1_ok={top1_ok}).")
